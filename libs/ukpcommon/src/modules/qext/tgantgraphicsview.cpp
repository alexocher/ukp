    ...
    m_year(year ? year : QDate::currentDate().year()), m_currentDay(curday), m_headerHeight(0), m_columnWidth(0), m_rowHeight(0), m_contentDraw(cdAll)
{
...
#ifdef DEBUG_INFO
    std::cerr << " fillHeaderDatas end " << std::endl;
#endif
    ...
    {
        sceneTable = new QGraphicsScene();
        //sceneTable = new QGraphicsScene(prnt);
        gvTable = new QGraphicsView(sceneTable);
        //gvTable = new QGraphicsView(prnt);
        sceneGant = new QGraphicsScene();
        //sceneGant = new QGraphicsScene(prnt);
        gvPlan = new QGraphicsView(sceneGant);
        //gvPlan = new QGraphicsView(prnt);
        scene = new QGraphicsScene();
        //gv = new QGraphicsView(scene, prnt);
        gv = new QGraphicsView(scene,this);

    }

    gvPlan->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    gvTable->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
...
#ifdef DEBUG_INFO
        //    position = qscrollbarVertgvPlan->sliderPosition();//->SliderPosition();
        //std::cerr << " !!!! qscrollbarHoriz определен !!!! "  << std::endl;
#endif
...
*/
    ...
    //qscrollbarVertgvPlan->setMaximum(m_rowHeight*rowcount  - qscrollbarVertgvPlan->pageStep() );
     if (m_rowHeight*rowcount + m_rowHeight - height  + headerHeight  > 0 )
...
#ifdef DEBUG_INFO
             std::cerr << " соединение с qscrollbarVert установлено " << std::endl;
#endif
            }
    ...
    if (prnt_gv){
        ...
        if (qscrollbarHoriz!=NULL){
            ...
            //qscrollbarHoriz->setSliderDown(false);
             ...
             //    position = qscrollbarVertgvPlan->sliderPosition();//->SliderPosition();
            std::cerr << " !!!! qscrollbarHoriz определен !!!! "  << std::endl;
            int min = this->minimumWidth(); int max = this->maximumWidth();// minimumWidth ()
...
#ifdef DEBUG_INFO
         std::cerr << " соединение с qscrollbarHoriz установлено " << std::endl;
#endif
    }
