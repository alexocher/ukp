...
//#include <QApplication> // для ProgressBar
#include <QProgressDialog> // для ProgressBar
#include <QProgressBar> // для ProgressBar
...
void      PlanProduct(int &l);
void      PlanProductEndMax(int &l);
int       Plan(TCarryTaskList &tasks, QString &errs);
...
{
    // По номеру рабочего часа года (от 0 до 365*8) найти астрономическую дату начала астрономического часа в формате QDateTime.
...
int DateTimeToNumWorkHourYear(QDateTime &dt) // перейти от даты QDateTime к номеру рабочего часа года
{
        ...
        for(int i=0; i<=1; i++){ // цикл перебора продукций (двух планов): План выпуска продукции (i=0) и План ОРД (i=1)
            ...
            foreach (TCarryProcedure *proc, plan->procedures()){ // цикл перебора процедур
                ...
                foreach (TCarryWork *work, proc->works()){ // цикл перебора работ
                    ...
                    //такую работу пропустить и не загружать в вектор job. Такая работа не будет участвовать в планировании
                    ...
                    // Заполнить минимально разрешенное время начала работы
                    //datetime = QDateTime(work->dtPlanBegin()->date(), work->dtPlanBegin()->time());
                    //jobtemp.begin = DateTimeToNumWorkHourYear(datetime);  // минимально возможное начало работы. Ном. раб. часа года (от 0 до 365*8)
                    jobtemp.beginmin = 0;  //??? НАДО СЧИТАТЬ минимально возможное начало работы. Ном. раб. часа года (от 0 до 365*8)

                    // Заполнить максимально разрешенное время конца работы
                    //datetime = QDateTime(work->dtPlanBegin()->date(), work->dtPlanBegin()->time());
                    //jobtemp.begin = DateTimeToNumWorkHourYear(datetime);  // минимально возможное начало работы. Ном. раб. часа года (от 0 до 365*8)
                    jobtemp.endmax = 0;  //??? НАДО СЧИТАТЬ максимально возможный конец работы. Ном. раб. часа года (от 0 до 365*8)
                    ...
                    if(jobtemp.protection){ // защищенная работа. В ней ничего менять нельзя
                        ...
                        jobtemp.begin = DateTimeToNumWorkHourYear(datetime);  // начало работы. Номер рабочего часа года (от 0 до 365*8)

                        // Заполнить время конца работы
                        datetime = QDateTime(work->dtPlanEnd()->date(), work->dtPlanEnd()->time());
                        jobtemp.end = DateTimeToNumWorkHourYear(datetime);  // начало работы. Номер рабочего часа года (от 0 до 365*8)
...
void PlanJob(int &l, int &m, int &n) // спланировать работу
{
    ...
    for(int ttek = tbeginplan;  ; ++ttek){ // бесконечный цикл перебора рабочих часов года
        ...
        if(m!=0 && n==0){  // это начальная работа процедуры. Надо найти конец последней работы предыдущей процедуры
            ...
            if(ttek <= job[l][m-1][klast].end) continue; // пропустить ttek
        }
...
void PlanProduct(int &i){ // спланировать продукцию без заданного максимального времени конца
...
void PlanProductEndMax(int &i){ // спланировать продукцию с заданным максимальным временем конца
    // Спланировать продукцию у которой задано максимально разрешенное время конца (т.е. не равно 0).
    //При этом минимальное время начала м.б. задано или не задано (т.е. равно 0 или не равно 0).
    //Поле beginmin будем использовать, что бы смещать работы "вправо", пока не достигнем максимално разрешенного времени
    //конца продукции. Затем beginmin восстановить

    int beginminold = job[i][0][0].beginmin; // запомнить минимально разрешенное время начала продукции
    int endmax      = job[i][0][0].endmax;   // заданное максимально разрешенное время конца процедуры (оно не равно 0)
    int beginmintek = job[i][0][0].beginmin; // текущее время начала продукции, которое будем менять
    int tekendproduct;                       // текущее время окончания продукции (для сравнения в итерациях)

    do { // один раз спланировать обязательно, что бы найти конец продукции
        PlanProduct(i); // спланировать продукцию без учета максимально разрешенного времени конца (для нахождения конца продукции)
        tekendproduct = job[i][job[i].size()-1][job[i][job[i].size()-1].size()-1].end; // время конца i-ой продукции
        // Проверка. Если еще не достигли максимально разрешенного времени конца продукции, то увеличить beginmin и продолжить планирование
        if(tekendproduct < endmax) {
            ++beginmintek; // сместить продукцию вправо на 1 единицу
        ...
        // Проверка. Если beginmintek превысил endmax, то прекратить поиск и выйти из цикла. Возможно вся продукция защищена и "вправо"
        //не движется. Это и защита от зацикливания при некорректных данных
        if(beginmintek > endmax){
            // Восстановить первоначальный beginminold во всех работах продукции при неуспешном поиске
            for(int j=0; j<(int)job[i].size(); ++j){ // цикл перебора работ текущей продукции
                for(int k=0; k<(int)job[i][j].size(); ++k){ // цикл перебора работ
                    job[i][j][k].beginmin = beginminold; // восстановить минимально разрешенное время начала продукции
                } // конец цикла перебора работ
            } // конец цикла перебора работ текущей продукции
            PlanProduct(i); // спланировать продукцию, игнорируя максимально допустимое время конца продукции
            break;
        }

    } while (tekendproduct < endmax);

    // Восстановить первоначальный beginminold во всех работах продукции при успешном поиске
    for(int j=0; j<(int)job[i].size(); ++j){ // цикл перебора работ текущей продукции
        for(int k=0; k<(int)job[i][j].size(); ++k){ // цикл перебора работ
            job[i][j][k].beginmin = beginminold; // восстановить минимально разрешенное время начала продукции
        } // конец цикла перебора работ
...
int Plan(TCarryTaskList &tasks, QString &errs) // основная процедура
{
    ...
    // Создать ProgressDialog (% не показывать. Только сам процесс, т.к. количество итераций заранее не известно)
    pprd = new QProgressDialog("Выполнение", "&Cancel", 0, 0);
    pprd->setMinimumDuration(300); // не показывать окно ProgressDialog, если программа выполняется менее 300 милисекунд
    pprd->setWindowTitle("Планирование");
    pprd->setValue(0); // установить значение 0 и менять его не будем. Нужна только индикация, что процесс продолжается
    //pprd->setCancelButton(false); // удалить кнопку Cancel
...
//    pprd->setBar(b);
    ...
    // Найти рабочий час года начала планирования (это аналог текущей даты)
    QDateTime currentDateTime(QDate::currentDate(), QTime::currentTime()); // текущие: дата и время
    tbeginplan = DateTimeToNumWorkHourYear(currentDateTime);               // текущий рабочий час года (от 0 до 365*8)

    FillHoliday();                               // заполнить календарь предприятия (выходные и праздничные дне) рабочими часами года
    ...
    for(int i=0; i<(int)job.size(); i++){ // цикл перебора продукций
        ...
        if(job[i][0][0].endmax) // Да, есть максимально разрешенное время конца продукции
            PlanProductEndMax(i);   // планирование продукции, в которой задано максимально разрешенное время конца
        else PlanProduct(i);        // планирование продукции, в которой не задано максимально разрешенноое время конца. Обычное планирование
