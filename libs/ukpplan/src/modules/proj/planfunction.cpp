...
{
    // здесь собраны все начальные проверки данные объекта tasks на ошибки, которые не позволят провести планирование
    //После отладки эту функцию надо будет удалить
    ...
    return 0; // норм. завершение функции
}

void TestPlan(QString &errs) // протестировать  сформированный план
{
    // Функция тестирует сформированный план на ошибки:
    //- нарушение последовательности работ одной продукции (, если последующая работа начинается до завершения предыдущей)
    //- дублирование исполнителя (, если исполнитель одновременно выполняет несколько работ в один момент времени)
    ...
    for(int i=0; i<(int)job.size(); ++i){ // цикл перебора продукций
        for(int j=0; j<(int)job[i].size(); ++j) { // цикл перебора работ
            executor_type executor = job[i][j].executor;// запомнить данные по проверяемой работе
...
int TestData(QString &errs) // тест считанных данных из объекта tasks
{
    ...
    if(executor.size()==0) {
        ...
        errs += QString("<br>Ошибка. Вектор executor (сведения об исполнителях) пустой<b>%1</b>").arg("");
        return 1;
    }
    ...
    for(int i=0; i<(int)executor.size(); ++i){
        if(executor[i].code==0) {
            ...
            errs += QString("<br>Ошибка. Вектор executor, поле code=<b>%1</b>").arg("0");
            return 2;
        }
    }
    ...
    foreach (TCarryTask *curTask,tasks) // цикл перебора проэктов
    {
        ...
        for(int i=0; i<=1; i++){ // цикл перебора двух продуктов (планов): План выпуска продукции (i=0) и План ОРД (i=1)
            ...
            foreach (TCarryProcedure *proc,plan->procedures()) // цикл перебора процедур
            {
                ...
                foreach (TEmployee *empl, proc->possibleEmployees()) qDebug()<<"            possibleEmployees (people) id="<<empl->id();
            ...
            foreach (TCarryProcedure *proc,plan->procedures()) // цикл перебора процедур
            {
                ...
                foreach (TCarryWork *work,proc->works()) // цикл перебора работ
                {
                    ...
                    if(!work->isVolatile()){ // работа защищена (false)
                        ...
                        //if(proc->employee()->id() == 0) {continue;    // пропустить защищенную работу с незаполненным исполнителем
                            qDebug()<<"работа id:"<<work->id()<<"нет исполнителя";
                        }
                    ...
                    jobtemp.time      = work->planPeriod();  // время работы, день
                    jobtemp.flcorrect = work->isVolatile();  // признак "можно изменять"

                    if(jobtemp.flcorrect){ // если работу "можно изменять",т.е. планировать, то обнулить ее данные
                        jobtemp.begin         = 0;  // время (день) начала работы??? Надо доделать
                        jobtemp.end           = 0;  // время (день) окончания работы??? Надо доделать
                        jobtemp.executor.code = 0;
                        jobtemp.executor.rest = {};
                    }
                    else{ // если у работы стоит признак "не изменять", т.е. работа уже спланировыана.
                        QDate beginQDate = work->dtPlanBegin()->date();
                        int beginint = beginQDate.dayOfYear() - 1; // номер дня должен начинаться с 0
                        jobtemp.begin = beginint; // день начала работы
                        //qDebug()<<"begin"<<beginQDate<<beginint;

                        QDate endQDate = work->dtPlanEnd()->date();
                        int endint = endQDate.dayOfYear() - 1; // номер дня должен начинаться с 0
                        jobtemp.end = endint; // день конца работы
                        //qDebug()<<"end"<<endQDate<<endint;

                        // если есть испольнитель, то записать код исполнителя. В качестве кода будет испорльзоваться id
                        if(proc->employee()) jobtemp.executor.code = proc->employee()->id();
                    }

                    //qDebug()<<"Cписок пустой possibleEmployees="<<proc->possibleEmployees().empty();
                    ...
                    if(proc->possibleEmployees().empty()){ // Да, пустй. Надо добавить одного фиктивного возможного исполнителя для процедуры
                        ...
                        // записать календарь фиктивного исполнителя
...
void FillTek(int &l, int &m, std::vector<time_type> &tek){ // заполнить массив контрольных точек
    ...
    //спланированных работ исполнителя job[l][m].executor

    time_type tempjob; // временная работа (только для заполнения вектора tek)
...
{ // Спланировать работу. Найти (для работы job[l][m]) время начала и конца с учетом календаря (личного и календаря предприятия) и с учетом ограничений
    ...
    if(job[l][m].flcorrect==false) return; // ВАЖНО проверить это??? Возникает ошибка

    std::vector<time_type>tek; // объявить вектор tek (для хранения tbegin и tend уже спланированных работ исполнителя)
    ...
    for(int kt=0; kt<(int)tek.size(); ++kt){ // цикл перебора КТ
        ...
        FindBeginEndJob(job[l][m].executor, tstart, job[l][m].time, tbegin, tend); // будут найдены tbegin и tend
...
int Plan(TCarryTaskList &tasks, QString &errs) // основная процедура
{
    ...
    if(TestData(errs)!=0) return 1; // проверить исходные данные, считанные из БД
...
stop1: // выход из цикла планирования в случае завершения планирования нужной работы job[i][j]
        ...
        // т.к. лучший исполнитель найден, то окончательно спланировать работы с найденным лучшим исполнителем
