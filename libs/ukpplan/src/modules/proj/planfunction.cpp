...
// Минимально разрешенное время начала продукции - время, раньше которого выполнение продукции не начнется. Позже - может.
// Максимально разрешенное время конца продукции - время, до которого продукции должна быть выполнена.
...
// Планировать можно только начиная с текущего рабочего часа. Планирование ранее, чем текущий рабочий час - невозможно.
//
// АЛГОРИТМ обработки beginmin и endmax (заданы или не заданы):
// - beginmin - нет, endmax - нет. Планирование начинается с текущего рабочего часа. beginmin и endmax - не учитываются.
// - beginmin - да,  endmax - нет. Планирование начинается с beginmin. endmax - не учитывается. Текущий рабочий час - не учитывается.
// - beginmin - нет, endmax - да.  Планирование заканчивается в endmax. Если найденное начало планирования меньше текущего рабочего часа, то
//                                 прервать планирование и сообщить об этом. beginmin - не учитывается.
// - beginmin - да,  endmax - да.  Планирование начнется с beginmin. Если найденный конец планирования превысит endmax, то прервать планирование
//                                 и сообщить об этом. Текущий рабочий час - не учитывается.
...
int Plan(TCarryTaskList &tasks, QString &errs) // основная процедура
{
    ...
    for(int i=0; i<(int)job.size(); i++){ // цикл перебора продукций
        ...
        if(job[i][0][0].endmax){ // Да, есть максимально разрешенное время конца продукции
            if(PlanProductEndBegin(i, errs)) return 1;   // обратное планирование продукции (задано максимально разрешенное время конца)
        }
        else {
            if(PlanProductBeginEnd(i, errs)) return 1;  // прямое планирование продукции (не задано максимально разрешенноое время конца)
        }

        // Проверка. Рассчитанное время начала продукции не может быть меньше времени начала планирования - tbeginplan (нельзя планировать в "прошлом")
        if(job[i][0][0].begin < tbeginplan){
            numerror ++; // порядковый номер ошибки
            errs += QString("<br><b>%1</b>. Планирование прервано. Рассчитанное время начала продукции меньше текущего времени для проекта: %2").arg(numerror).arg(job[i][0][0].projectnum);
            return 1;
        }

        // Проверка. Рассчитанное время начала продукции не может быть меньше минимально разрешенного времени начала продукции
        if(job[i][0][0].beginmin){ // если задано минимально разрешенное время начала продукции
            if(job[i][0][0].begin < job[i][0][0].beginmin){ // если рассчитанное время начала продукции меньше заданного времени начала продукции
                numerror ++; // порядковый номер ошибки
                errs += QString("<br><b>%1</b>. Планирование прервано. Рассчитанное время начала продукции меньше минимально заданного для проекта: %2").arg(numerror).arg(job[i][0][0].projectnum);
                return 1;
            }
        }

        // Проверка. Рассчитанное время конца продукции не может быть больше максимально разрешенного времени конца продукции
        if(job[i][0][0].endmax){ // если задано максимально разрешенное время конца продукции
            if(job[i][0][0].end > job[i][0][0].endmax){ // если рассчитанное время конца продукции больше заданного времени конца продукции
                numerror ++; // порядковый номер ошибки
                errs += QString("<br><b>%1</b>. Планирование прервано. Рассчитанное время конца продукции больше максимально заданного для проекта: %2").arg(numerror).arg(job[i][0][0].projectnum);
                return 1;
            }
        }
