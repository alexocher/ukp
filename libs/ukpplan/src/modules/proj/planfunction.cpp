...
// Термин "в прошлом" означент время меньше текущего времени. Планировать "в прошлом" - нельзя.
//
...
int TestData(QString &errs) // тестировать job до планирования
{
...
void CorrectionProcedure() // проверить возможных исполнителей процедур
{
    ...
    // Это перестраховка от возможных ошибок манипуляций оператора с признаком защищенностью работ.
        ...
        for(int j=0; j<(int)job[i].size(); ++j){ // цикл перебора процедур текущей продукции
            ...
            if(flprotection){ // была найдена защищенная работа
                for(int k=0; k<(int)job[i][j].size(); ++k){ // цикл перебора работ текущей процедуры
                    ...
                    //qDebug()<<"Обнулен список возможных исполнителей";
