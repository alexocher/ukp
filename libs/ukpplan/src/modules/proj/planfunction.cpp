...
// - beginmin - да,  endmax - нет. Планирование начинается с beginmin. endmax b текущий рабочий час - не учитываются.
...
namespace {
    ...
    struct job_type { // структура работы
        ...
        int                    endmax;       // максимально разрешенное время (рабочий час года) конца работы (из "Закончить до...") (от 0 до 365*8). Если
...
int Plan(TCarryTaskList &tasks, QString &errs) // основная процедура
{
    ...
    for(int i=0; i<(int)job.size(); i++){ // цикл перебора продукций
        ...
        // Планирование продукции: прямое или обратное? (В зависимости от того, что заданно ли максимально разрешенное время конца продукции)
...
bool TModulePlans::createCarryPlans(TCarryTaskList &tasks, QString &errs)
{
    ...
    Progress();  // Создать ProgressDialog и внутри него ProgressBar (для варианта, если планирование длится очень долго)
