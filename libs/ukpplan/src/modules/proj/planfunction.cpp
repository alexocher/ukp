...
// Минимально разрешенное время продукции - гарантированное время, раньше которого выполнение продукции не начнется. Позже - может.
// Максимально разрешенное время продукции - время, до которого продукции должна быть выполнена. Но это не гарантируется
...
namespace {
    ...
    int   flprint         = 0;       // флаг вывода отладочной информации в консоль: 0-ничего; 1-ошибки,план; 2-ошибки,план,job; 3-ошибки,план,job,tasks
    ...
    QVector<int>holidey;                      // вектор праздничные и выходных дней (рабочих часов) (календарь предприятия). Начиная с 0
}
...
int       PlanJob(int &l, int &m, int &n, QString &errs);
int       PlanProduct(int &l, QString &errs);
int       PlanProductEndMax(int &l, QString &errs);
int       Plan(TCarryTaskList &tasks, QString &errs);
...
{
    // По номеру рабочего часа (n) года (от 0 до 365*8) найти астрономическую дату начала астрономического часа в формате QDateTime.
    // Если str == "begin", то найти астрономическое время начала рабочего часа,
    //если str == "end", то найти астрономическое время НАЧАЛА СЛЕДУЮЩЕГО рабочего часа после последнего часа работы.
    //Например: time = 2; begin = 13:00:00; end = 15:00:00 (, а не 14:59:59)
            ...
            if(curTask->dtMinBegin()){ // если минимально возможное время начала проекта задано
                if(curTask->dtMaxEnd()){ // если максимально возможное время конца проекта задано
                    if(curTask->dtMinBegin() > curTask->dtMaxEnd()){ // если время "начала" больше времени "конца"
                        qDebug()<<"Ошибка. Заданное время начала проекта больше времени конца. curTask->dtMinBegin()="<<QDateTime(curTask->dtMinBegin()->date(), curTask->dtMinBegin()->time()).toString("d.M.yyyy")
            ...
            foreach (TCarryProcedure *proc, plan->procedures()){ // цикл перебора процедур
                ...
                foreach (TCarryWork *work, proc->works()){ // цикл перебора работ
                    ...
                    foreach (TEmployee *empl, proc->possibleEmployees()) { // цикл перебора возможных исполнителей
                        ...
                        // Заполнить executortemp.rest (календарь исполнителя) нерабочими часами исполнителя. Учесть год календаря и год планирования
                        //???...
                        if(empl->calendar()){
                            //qDebug()<<"есть календарь empl->calendar()->id()"<<empl->calendar()->id();
                            //empl->calendar()->fullRestDates();
                            //qDebug()<<"empl->calendar()->fullRestDates()"<<empl->calendar()->fullRestDates();
                            //qDebug()<<empl->calendar()->toStr();
...
void PrinTCarryTask(TCarryTaskList &tasks) // вывести в консоль объект tasks
{
    ...
    foreach (TCarryTask *curTask,tasks) // цикл перебора проэктов
    {
        ...
        qDebug()<<"Проект dtMinBegin="<<curTask->dtMinBegin();
        qDebug()<<"Проект dtMaxEnd="<<curTask->dtMaxEnd();
...
int PlanJob(int &l, int &m, int &n, QString &errs) // спланировать работу
{
...
int PlanProduct(int &i, QString &errs){ // спланировать продукцию без заданного максимального времени конца
    ...
    for(int j=0; j<(int)job[i].size(); j++){ // цикл перебора процедур
        ...
        for(int indexecutor = 0; indexecutor<(int)job[i][j][0].people.size(); indexecutor++){ // цикл перебора возможных исполнителей тек. процедуры
            ...
            for(int k=0; k<(int)job[i][j].size(); k++){
                if(PlanJob(i, j, k, errs) !=0) return 1; // планирование работ процедуры
            }
...
int PlanProductEndMax(int &i, QString &errs){ // спланировать продукцию с заданным максимально возможным временем конца
    // Спланировать продукцию у которой задано максимально разрешенное время конца (т.е. оно не равно 0).
    //При этом минимальное время начала м.б. задано или не задано (т.е. равно 0 или не равно 0).
    //Поле beginmin будем использовать, что бы смещать работы "вправо", пока не достигнем максимально разрешенного времени
    //конца продукции. Затем beginmin восстановить.
    // Это самая долгая процедура из всех.

    int beginminold = job[i][0][0].beginmin; // запомнить минимально разрешенное время начала продукции для восстановления
    int endmax      = job[i][0][0].endmax;   // заданное максимально разрешенное время конца процедуры (оно не равно 0)
    int tekendproduct;                       // текущее время окончания продукции (для сравнения в итерациях)    
    int beginmintek;                         // текущее время начала планирования

    // Если минимально заданное время начала продукции есть, то используем его для начала итераций. Иначе используем
    //текущий рабочий час года
    if(job[i][0][0].beginmin) beginmintek = job[i][0][0].beginmin; // текущее время начала продукции, которое будем менять
    else beginmintek = tbeginplan; // начать проверки с текущего рабочего часа года

    if(PlanProduct(i, errs) != 0) return 1; // спланировать продукцию для нахождения конца продукции
    tekendproduct = job[i][job[i].size()-1][job[i][job[i].size()-1].size()-1].end; // время конца i-ой продукции

    // Проверка. Найденный конец продукции уже превысил максимально заданное время конца продукции?
    if(tekendproduct >= endmax) return 0; // Да. Тогда завершить функцию. Никаких итераций делать не надо. Невозможно завершить работу в endmax

    // Найти beginmintek - грубую величину начала продукции, что бы завершить продукцию, примерно, в endmax. Затем будем уточнять beginmintek.
    //Так будет быстрее
    beginmintek = beginmintek + (endmax - tekendproduct); //, где (endmax - tekendproduct) - разница между заданным и полученным концом продукции
    for(int j=0; j<(int)job[i].size(); ++j){ // цикл перебора работ текущей продукции
        for(int k=0; k<(int)job[i][j].size(); ++k){ // цикл перебора работ
            job[i][j][k].beginmin = beginmintek; // записать новый tekbeginmin во все работы продукции
        } // конец цикла перебора работ
    } // конец цикла перебора работ текущей продукции
    if(PlanProduct(i, errs) != 0) return 1; // спланировать продукцию для нахождения конца продукции
    tekendproduct = job[i][job[i].size()-1][job[i][job[i].size()-1].size()-1].end; // время конца i-ой продукции
    // получили значение конца продукции близкого к заданному концу. Но не известно: спрва или слева. Надо проверять: вначале слева, потом справа
    //qDebug()<<"endmax"<<endmax<<"tekendproduct"<<tekendproduct;

    // Движение продукции вправо, пока не пересечем endmax (слева направо). Итерации продолжаются до тех пор, пока конец
    //продукции не пересечет гарантированно endmax
    while (tekendproduct < endmax){ // пока конец продукции слева от endmax
        //++beginmintek; // сместить продукцию вправо (на один шаг, равный одному рабочему часу года)
        beginmintek = beginmintek + kolworkhoursday; // сместить продукцию вправо (на один рабочий день). Так менее точно, но быстрее
        // Записать новый beginmintek (увеличенный на 1) во все работы продукции в поле beginmin
        for(int j=0; j<(int)job[i].size(); ++j){ // цикл перебора работ текущей продукции
            for(int k=0; k<(int)job[i][j].size(); ++k){ // цикл перебора работ
                job[i][j][k].beginmin = beginmintek; // записать новый tekbeginmin (увеличенный на 1) во все работы продукции
            } // конец цикла перебора работ
        } // конец цикла перебора работ текущей продукции
        if(PlanProduct(i, errs) != 0) return 1; // спланировать продукцию для нахождения конца продукции
        tekendproduct = job[i][job[i].size()-1][job[i][job[i].size()-1].size()-1].end; // время конца i-ой продукции
        //qDebug()<<"endmax"<<endmax<<"tekendproduct"<<tekendproduct<<"итерации слева направо";

    } // конец цикла while (tekendproduct < endmax)
    // после выхода из цикла конец продукции находится в т. endmax или рядом справа

    // Движение продукции влево, пока не пересечем endmax (справа налево). До первого приемлемого варианта
    while (tekendproduct >= endmax){ // пока конец продукции справа от endmax
        //beginmintek = beginmintek - 1; // сместить продукцию влево (на один шаг, равный одному рабочему часу года)
        beginmintek = beginmintek - kolworkhoursday; // сместить продукцию влево (на один рабочий день). Так менее точно, но быстрее
        // Записать новый beginmintek (уменьшинный на 1) во все работы продукции в поле beginmin
        for(int j=0; j<(int)job[i].size(); ++j){ // цикл перебора работ текущей продукции
            for(int k=0; k<(int)job[i][j].size(); ++k){ // цикл перебора работ
                job[i][j][k].beginmin = beginmintek; // записать новый tekbeginmin (увеличенный на 1) во все работы продукции
            } // конец цикла перебора работ
        ...
        if(PlanProduct(i, errs) != 0) return 1; // спланировать продукцию для нахождения конца продукции
        tekendproduct = job[i][job[i].size()-1][job[i][job[i].size()-1].size()-1].end; // время конца i-ой продукции
        //qDebug()<<"endmax"<<endmax<<"tekendproduct"<<tekendproduct<<"итерации справо налево";
    } // конец цикла while (tekendproduct >= endmax)
    // после выхода из цикла конец продукции находится в т. endmax или рядом слева. То, что надо.
    //qDebug()<<"endmax"<<endmax<<"tekendproduct"<<tekendproduct<<"итерации кончились";

    // На каждой итерации менялось значение beginmin. После окончания всех итераций надо его восстановить из сохраненного beginminold
    for(int j=0; j<(int)job[i].size(); ++j){ // цикл перебора работ текущей продукции
        for(int k=0; k<(int)job[i][j].size(); ++k){ // цикл перебора работ
            job[i][j][k].beginmin = beginminold; // восстановить минимально разрешенное время начала продукции
        } // конец цикла перебора работ
    } // конец цикла перебора работ текущей продукции
    return 0;
}
...
int Plan(TCarryTaskList &tasks, QString &errs) // основная процедура
{
    ...
    // Найти tbeginplan - время начала планирования. Это начальный час, с которого можно будет планировать.
    //Это текущий рабочий час года (аналог текущей даты)
    ...
    FillHoliday();                               // заполнить календарь предприятия (выходные и праздничные дни) рабочими часами года
    ...
    // Создать ProgressDialog (pprd) и внутри него ProgressBar (pprb). Если планирование длится долго, предусмотреть возможность
    //принудительного прекращения планирования. Такое м.б., если во многих проектах будет задано максимально возможное время конца
    ...
    for(int i=0; i<(int)job.size(); i++){ // цикл перебора продукций
        ...
        if(job[i][0][0].endmax){ // Да, есть максимально разрешенное время конца продукции
            if(PlanProductEndMax(i, errs) != 0) return 1;   // планирование продукции, в которой задано максимально разрешенное время конца
        }
        else {
            if(PlanProduct(i, errs) != 0) return 1;  // планирование продукции, в которой не задано максимально разрешенноое время конца. Обычное планирование
        }
