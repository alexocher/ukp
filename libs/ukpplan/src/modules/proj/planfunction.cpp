...
// Приоритет продукции (по-умолчанию = 0) позволяет управлять последовательность планирования продукции. Чем выше приоритет, тем быстрее будет
...
// В каждой процедуре есть начальная работа, остальные работы называются внутренними работами процедурами.
...
//только с меньшим порядковым номером (индексом). Для этого  может потребоваться выполнение упорядочиваня процедур в шаблоне.
...
namespace {
    ...
    struct job_type { // структура работы
        ...
        QVector<int>           prevprocedurenum; // список номеров (num) опорных процедур
        ...
        int                    beginmin;         // заданное мин. разрешенное время (рабочий час года) начала работы (из "Планировать проект с...") (от 0 до 365*8)
                                                 //По умолчанию = 0
...
//    } // конец цикла перебора продукций
    ...
    //только меньшие индексы процедур, чем индекс проверяемой процедуры
    for(int i=0; i<(int)job.size(); ++i){ // цикл перебора продукций
        for(int j=0; j<(int)job[i].size(); ++j){ // цикл перебора процедур текущей продукции
            for(int k=0; k<(int)job[i][j].size(); ++k){ // цикл перебора работ текущей процедуры
                for(int g=0; g<(int)job[i][j][k].prevprocedureind.size(); ++g){ // цикл перебора индексов опорных процедур
                    ...
                    if(tekind > j) { // если "младшая" процедура ссылается на "старшую" это ошибка
                        numerror ++; // порядковый номер ошибки
                        errs += QString("<br><br><b>%1</b>. Индекс опорной процедуры превышает индекс самой процедуры: %2. %3 / %4. %5 / %6. %7").
                                arg(numerror).
                                arg(job[i][j][0].projectnum).  arg(job[i][j][0].projectname).
                                arg(job[i][j][0].productnum).  arg(job[i][j][0].productname).
                                arg(job[i][j][0].procedurenum).arg(job[i][j][0].procedurename);
...
//    //+ 06.07.17. После введения "параллельности", принцип последовательности не будет выполняться, как раньше
...
//    //-
    ...
    // Конец спланированной работы должен превышать начало той же работы (равенство начала и конца допускается, если работа длится 1 час)
    for(int i=0; i<(int)job.size(); ++i){ // цикл перебора продукций
        for(int j=0; j<(int)job[i].size(); ++j){ // цикл перебора процедур
            for(int k=0; k<(int)job[i][j].size(); ++k) { // цикл перебора работ
                if(job[i][j][k].begin > job[i][j][k].end){
...
bool TestJobOrderBeginEnd(int &l, int &m, int &n, int &ttek) // проверка выполнения принципа последовательности работы (прямое планирование)
{
    ...
    //последовательно. А процедуры могут выполняться и последовательно и параллельно, т.е. могут начинаться в любое время.

    // Проверка. Проверяемая работа начальная в процедуре?
    if(n == 0){ // Начальная работа процедуры. Надо анализировать опорные процедуры
    ...
    else { // Внутренняя работа процедуры. Надо проверить на окончание предыдущей работы в рамках той же процедуры
        if(ttek <= job[l][m][n-1].end) return false; // ttek не подходит (вернуть false)
        return true; //ttek подходит (вернуть true). Работа job[l][m][n] может начаться во время ttek
    }
...
bool TestJobOrderEndBegin(int &l, int &m, int &n, int &ttek) // проверка выполнения принципа последовательности работы (обратное планирование)
{
    ...
    //Если это не последняя работа процедуры, то для проверки нужна следующая (уже спланированная) работа той же процедуры
    int klast = job[l][m].size()-1; // индекс k последней работы в текущей процедуре, которой принадлежит работа job[l][m][n]
    if(n!=klast) if(ttek >= job[l][m][n+1].begin) return false; // пропустить ttek
    ...
    if(n==klast && m!=job[l].size()-1)  {  // это последняя работа процедуры, но процедура не последняя. Надо найти начало начальной работы следующей процедуры
        if(ttek >= job[l][m+1][0].begin) return false; // пропустить ttek
    }

    return true; // принцип последовательности работ - выполняется
}
...
{
    //??? Не ясно до конца, как работать с началом года? Если разные проекты в базе будут иметь разные года? Нужна единая шкала времени
