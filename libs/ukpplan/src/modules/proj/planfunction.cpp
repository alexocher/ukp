...
// Все работы одной процедуры может выполняет только один исполнитель. Возможных исполнителей м.б. несколько.
...
namespace {
    ...
    int   flprint         = 0;       // флаг вывода отладочной информации в консоль: 0-ничего; 1-ошибки,план; 2-ошибки,план,job;
                                     //3-ошибки,план,job,tasks, paraljob
...
bool TestJobOrderEndBegin(int &l, int &m, int &n, int &ttek) // проверка выполнения принципа последовательности работы (обратное планирование)
{
    ...
    else { // не последняя работа процедуры. Надо проверить на начало следующей (уже спланирпованной) работы той же процедуры
