...
// Терминология:
// Номер рабочего часа года м.б. от 0 до 365*8. В году 365 суток по 8 часов. Считаем, что в сутках только 8 часов.
...
// Максимально разрешенное время конца продукции - время, до которого продукции должна быть выполнена. Если это требование невыполнимо - планирование прерывается
...
// Планировать можно только начиная с текущего рабочего часа. Планирование ранее, чем текущий рабочий час (планирование в прошлом) - невозможно.
...
//
// "Обычная" процедура (ОП) и "параллельная" процедура (ПП). ПП позволяет выполнять последующие процедуры (одну или несколько, ПП или ОП)
//параллельно текущей ПП. В этом случае не всегда будет выполняться жесткий принцип последовательности работ одной продукции. Унивесальное правило - любая
//процедура (ПП или ОП) может начинаться только после окончания ближайшей предыдущей ОП. Несколько ПП и ОП могут выполняться параллельно.
//Для того, что бы в ПП, осуществилась действительно паралллельность необходимо не менее 2-ух возможных исполнителей процедуры. Если
//возможный исполнитель будет один, то никакой параллельности не будет, т.к. один исполнитель не может одновременно выполнять несколько
//работ. При этом, ошибки не будет, просто процедуры будут выполняться последовательно. Все работы принадлежащие параллельной процедуре будут иметь
//признак paral=true и называются параллельные работы (ПР). Все работы, принадлежащие обычной процедуре будут иметь признак paral=false и называются
//обычные работы (ОР).
//
...
namespace {
    ...
    int   tbeginplan      = 0;       // номер текущего рабочего часа года (начиная с 0), с которого начнется планирование (аналог текущей даты)
    int   numerror        = 0;       // порядковый номер ошибки для сообщения об ошибке
    ...
    int   flprint         = 3;       // флаг вывода отладочной информации в консоль: 0-ничего; 1-ошибки,план; 2-ошибки,план,job; 3-ошибки,план,job,tasks, paraljob
    QDateTime emptydate(QDate(2000,1,1), QTime(9,0)); // именно такая дата появляется, если очистить поле "Планировать с ..." и "Завершить до ...". Такая
                                     //дата считается "пустой" и не будет учитываться при планировании
    ...
    struct job_type { // структура работы
        ...
        int                    beginmin;      // заданное минимально разрешенное время (рабочий час года) начала работы (из "Планировать проект с...") (от 0 до 365*8)
                                              //По умолчанию = 0
        ...
        int                    endmax;        // заданное максимально разрешенное время (рабочий час года) конца работы (из "Закончить до...") (от 0 до 365*8). Если
                                              //задан 0 (по - умолчанию), то считать, что ограничения по концу - нет
        executor_type          executor;      // назначенный исполнитель работы (обязательно из списка people)
        QVector<executor_type> people;        // список возможных исполнителей работы (с пустым списком планирование невозможно)
        ...
        bool                   paral;         // принадлежность работы параллельной процедуре (true - параллельная, false - обычная).
    };

    QVector<QVector<QVector<job_type>>> job;  // вектор работ, содержащий всю основную информацию по всем работам
    QVector<executor_type> executor;          // вектор исполнителей (общий список из people всех работ)
}
...
void Progress() // настройка ProgressBar
{
    ...
    //Возможно, это будет необходимо при больших планах. Пока (при небольших планах) ProgressBar не виден.
...
QDateTime NumWorkHourYearToDateTime(int &n, QString &str) // перейти от номера рабочего часа года к QDateTime
{
    ...
    //Например, если работа начинается в 11:00 и ее продолжительность равна 1 часу, то конец работы будет 12:00.
...
int TestTCarryTaskBeforePlanning(TCarryTaskList &tasks, QString &errs) // проверить объект tasks до планирования
{
    ...
    foreach (TCarryTask *curTask,tasks) // цикл перебора проектов
    {
        ...
        for(int i=0; i<=1; i++){ // цикл перебора продукций (План выпуска продукции (i=0) и План ОРД (i=1))
            ...
            //??? Не ясно до конца, откуда год брать? Когда в базе будут планы разных годов.
                ...
                foreach (TCarryWork *work,proc->works()){ // цикл перебора работ
                    if(work){ // проверка на нулевой указатель
                        ...
                        if(!work->planPeriod()){
                            flerror = true; // признак наличия ошибки
                            numerror ++; // порядковый номер ошибки
                            errs += QString("<br><b>%1</b>. Не определена длительность работы: %2.%3.%4.%5").arg(numerror).arg(curTask->num()).arg(plan->num()).arg(proc->num()).arg(work->num());
                        }
                    }
                    else { // пустая работа
                        flerror = true; // признак наличия ошибки
                        numerror ++; // порядковый номер ошибки
                        errs += QString("<br><b>%1</b>. Пустая работа: %2.%3.%4.%5").arg(numerror).arg(curTask->num()).arg(plan->num()).arg(proc->num()).arg(work->num());
                    }
...
int TestPlan(QString &errs) // протестировать  сформированный план
{
    ...
    //Это проверка последовательности выполнения работ одной продукции
    ...
    for(int i=0; i<(int)job.size(); ++i){ // цикл перебора продукций
        for(int j=0; j<(int)job[i].size(); ++j){ // цикл перебора процедур
            for(int k=0; k<(int)job[i][j].size(); ++k) { // цикл перебора работ
                ...
                //if(job[i][j][k].paral >0) continue; // ??? ПОДУМАТЬ для параллельной работы не проверять последовательность (т.к. может нарушаться)
...
void SortingVectorJob() // сортировка вектора job
{
    ...
    //получать ресурсы исполнителей по остаточному принципу. Чем больше приоритет продукции ("выше" продукция), тем быстрее она будет выполнена
...
void PrintParalJob() //??? УДАЛИТЬ
{
//    // Вывести в консоль вектор job (до и после обработки параллельности)
//    qDebug()<<"-----------------------------------------";
//    QVector<QVector<QVector<job_type>>>::iterator prod = job.begin(); // итератор для перебора продукции
//    while (prod != job.end()) { // цикл перебора продукций
//        qDebug()<<"ПРОДУКЦИЯ. размер i-ой продукции (кол. процедур) prod->size()="<<prod->size();

//        QVector<QVector<job_type>>::iterator proc = prod->begin(); // итератор для перебора процедур
//        while (proc != prod->end()) { // цикл перебора процедур
//            qDebug()<<"     ПРОЦЕДУРА. размер j-ой процедуры (кол. работ) proc->size()="<<proc->size();

//            QVector<job_type>::iterator work = proc->begin(); // итератор для перебора продукции
//            while (work != proc->end()) { // цикл перебора работ
//                qDebug()<<"          РАБОТА. work->time="<<work->time<<"work->paral="<<work->paral;
//                work++;
//            } // конец цикла перебора работ
//            proc++;
//        } // конец цикла перебора процедур
...
void ParalJob() // обработка параллельных работ УДАЛИТЬ???
{
//    // Обработка "паралллельных работ" (ПР) в векторе job. Каждая процедура, содержащая ПР, будет разделена на 3 новых процедуры:
//    // - первая процедура будет содержать работы до ПР.
//    // - вторая процедура будет содержать только одну ПР.
//    // - третья процедура будет содержать работы после ПР.
//    //Общее количество работ останется прежним. Сохранятся id всех работ.
//    // За одну итерацию будет обрабатываться только одна ПР.
//    // В одной процедуре м.б. несколько ПР. Для их различия используется work->paral. 0-обычная работа; 1-ПР необработанная; 2-ПР обработанная.
//    //"Обработанная" означает выделение параллельной работы в отдельную процедуру.

//    bool prparaljob; // признак обработанной ПР. Завершить функцию, когда при очередной итерации не будет
//    //обработана ни одна ПР. К этому моменту все ПР должны будут выделены в отдельные процедуры

//    do { // итерация обработки ПР. Поиск первой встретившийся необработанной ПР в векторе job

//        prparaljob = false; // установить признак "не была обработана ПР". Используется для завершения работы функции

//        QVector<QVector<QVector<job_type>>>::iterator prod1; // указатель на продукцию, содержащую параллельную
//        QVector<QVector<job_type>>::iterator proc1; // указатель на 1-ую из 3-ех новых процедур

//        // Перебор всех работ через итераторы (, а не через индексы) для копирования и изменения (удаления) процедур
//        QVector<QVector<QVector<job_type>>>::iterator prod = job.begin(); // итератор продукции
//        while (prod != job.end()) { // цикл перебора продукций
//            QVector<QVector<job_type>>::iterator proc = prod->begin();    // итератор процедур
//            while (proc != prod->end()) { // цикл перебора процедур
//                QVector<job_type>::iterator work = proc->begin();         // итератор работ
//                while (work != proc->end()) { // цикл перебора работ
//                    // Проверка. Текущая ПР необработанная? Обработанная ПР пропускается
//                    if(work->paral == 1){ // найдена параллельная необработанная работа (в процедуре их м.б. несколько. Работаем только с первой)
//                        work->paral = 2; // сразу пометить ПР, как обработанную (позже, после удаления процедур этот указатель теряется)

//                        // Проверка. Если в процедуре единственная параллельная работа, то ничего не делать. И ее пропустить
//                        if(proc->size() != 1){ // если в процедуре количество работ не равно единицы

//                            // вставить 2 новые процедуры-копии. Это будет 1-ая, 2-ая. А 3-я процедура - это оригинал (, который копируется)
//                            proc = prod->insert(proc, 2, *proc); // после вставки итератор proc будет указывать на 1-ую из 3-ех новых процедур
//                            proc1 = proc; // запомнить указатель на 1-ую новую процедуру
//                            prod1 = prod; // запомнить указатель на продукцию , которой принадлежат все три новые процедуры
//                            prparaljob = true; // признак того, что в этой итерации была обработана параллельная работа

//                            // 1-ая процедура. В ней удалить параллельную работу и все обычные последующие работы.
//                            //Оставить только обычные работы ДО ПР
//                            bool pr = false; // признак того, что параллелельная работа (paral=2)еще не достигнута
//                            QVector<job_type>::iterator work1 = proc1->begin(); // итератор для перебора работ 1-ой процедуры
//                            while (work1 != proc1->end()) { // цикл перебора работ процедуры
//                                // Проверка. Если это обычная работа ДО параллельной работы, то ее не удалять
//                                if((work1->paral == 0) && (pr == false)) {
//                                    work1++;
//                                    continue; // оставить (не удалять) обычную работу ДО параллельной работы
//                                }
//                                // Проверка. Это параллельная работа, которую обрабатывем?
//                                if(work1->paral == 2){
//                                    pr = true;            // установить признак того, что параллелельная работа достигнута
//                                    proc1->erase(work1);  // удалить параллельную работу
//                                    continue;
//                                }
//                                // удллить после параллельной (paral=2) все обычные работы и если есть, то и параллельные необработанные работы (paral=1)
//                                proc1->erase(work1);
//                            } // конец цикла перебора работ процедуры

//                            // 2-ая процедура. В ней удалить все обычные работы, кроме параллельной
//                            //Оставить только одну ПР
//                            proc1++; // получить указатель на 2-ю поцедуру (из 3-ех)
//                            work1 = proc1->begin(); // итератор для перебора работ процедуры
//                            while (work1 != proc1->end()) { // цикл перебора работ процедуры
//                                // Проверка. Это обычная работа?
//                                if(work1->paral != 2) proc1->erase(work1);  // удалить обычную работу
//                                else work1++; // пропустить параллельную работу
//                            } // конец цикла перебора работ процедуры

//                            // 3-я процедура. В ней удалить все обычные работы до параллельной работы и саму параллельную работу
//                            //Оставить только обычные работы после ПР
//                            proc1++; // получить указатель на 3-ю процедуру (из 3-ех)
//                            pr = false; // признак того, что параллелельная работа еще не достигнута
//                            work1 = proc1->begin(); // итератор для перебора работ процедуры
//                            while (work1 != proc1->end()) { // цикл перебора работ процедуры
//                                // Проверка. Если это обычная работа ДО параллельной работы, то удалить обычную работу
//                                if((work1->paral == 0) && (pr == false)) {
//                                    proc1->erase(work1);      // удалить все обычные работы после параллельной
//                                    continue;
//                                }
//                                // Проверка. Это параллельная работа?
//                                if(work1->paral == 2){
//                                    pr = true;            // установить признак того, что параллелельная работа достигнута
//                                    proc1->erase(work1);  // удалить параллельную работу
//                                    continue;
...
//                            // Проверить 3 новые процедуры. Если есть пустые, то удалить их. Пустая процедура м.б., если параллельная работа
//                            //будет начальной (или последней) в процедуре. Сейчас итератор proc1 указывает на 3-ю процедуру
//                            proc1--; proc1--; // установить итератор на 1-ую процедуру
//                            if(proc1->empty()) prod1->erase(proc1); // если 1-ая процедура пустая, то удалить ее (и перейти ко 2-ой процедуре)
//                            else proc1++; // перейти ко 2-ой процедуре
//                            if(proc1->empty()) prod1->erase(proc1); // если 2-ая процедура пустая, то удалить ее (хотя, в ней всегда будет ПР)
//                            else proc1++; // перейти к 3-ей процедуре
//                            if(proc1->empty()) prod1->erase(proc1); // если 3-я процедура пустая, то удалить ее

//                            // Обработка ПР завершена. Выйти из всех циклов-переборов, т.к. за один цикл можно обрабатывать только одну ПР
//                            goto metka;
//                        }
//                    }
//                    work++;
...
//            prod++;
//        } // конец цикла перебора продукции
//metka:; // метка завершения всех циклов после обработки параллельной работы
//    } while (prparaljob); // конец цикла итераций. Завершить итерации (и функцию), если в ней не была обработана ПР
}
        ...
        for(int i=0; i<=1; i++){ // цикл перебора продукций (двух планов): План выпуска продукции (i=0) и План ОРД (i=1)
            ...
            foreach (TCarryProcedure *proc, plan->procedures()){ // цикл перебора процедур
                ...
                foreach (TCarryWork *work, proc->works()){ // цикл перебора работ
                    ...
                    //+
                    if(proc->num() == 3) jobtemp.paral = true;   // ??? УДАЛИТЬ. Только для отладки // параллельность работы
                    else                 jobtemp.paral = false;  // ??? УДАЛИТЬ. Только для отладки // параллельность работы
                    //-
...
void PrinTCarryTask(TCarryTaskList &tasks) // вывести в консоль объект tasks
{
    ...
    foreach (TCarryTask *curTask,tasks) // цикл перебора проэктов
    {
        ...
        if(curTask->dtMinBegin()) qDebug()<<"Проект dtMinBegin="<<curTask->dtMinBegin()->date()<<curTask->dtMinBegin()->time();
        else                      qDebug()<<"Проект dtMinBegin=...";

        if(curTask->dtMaxEnd()) qDebug()<<"Проект dtMaxEnd="<<curTask->dtMaxEnd()->date()<<curTask->dtMaxEnd()->time();
        else                    qDebug()<<"Проект dtMaxEnd=...";
...
int PlanJobBeginEnd(int &l, int &m, int &n, QString &errs) // прямое планирование работы
{
    ...
    // Проверка. Нажата кн. Прервать? При долгом планировании можно прервать планирование.
    ...
    for(int ttek = tbeginplan;  ; ++ttek){ // бесконечный цикл перебора рабочих часов года, начиная с текущего рабочего часа
        ...
        //СТАРЫЙ ВАРИАНТ, когда не было параллельных процедур и работ
        ...
        //НОВЫЙ ВАРИАНТ
        //ПРОВЕРКА. Работа не может начаться ранее, чем закончится ближайшая предыдущая обычная (не параллельная) работа
//        if(!((m==0) && (n==0))){ // для начальной работы продукции нет предшествующей работы. Проверка для нее не нужна
//            int tend = 0; // искомое время предыдущей обычной работы
//            bool find = false; //признак того, что нужную работу еще не достигли припереборе работ
//            // Цикл обратного перебора всех процедур и работ l-ой продукции, начиная с работы job[l][m][n]
//            for(int j=job[l].size()-1; j>=0; j--){ // обратный цикл перебора процедур
//                for(int k=job[l][j].size()-1; k>=0; k--){ // обратный цикл перебора работ
//                    if((j==m) && (k==n)){
//                        find = true; // признак, что теперь можно искать время конца
//                        continue; // пропустить саму проверяемую работу. А следующая будет предшествующая
//                    }
//                    if(job[l][j][k].paral > 0) continue; // пропустить ПР. Нужна только обычная работа
//                    if(find == true) { // признак указывает на то, что можно искать конец работы
//                        tend = job[l][j][k].end; // конец ближайшей предшествующей обычной работы
//                        goto metka22; // прервать дальнейший поиск. То что искали - нашли
//                    }
//                } // конец обратного цикла перебора работ
//            } // конец обратного цикла перебора процедур
//metka22:; // метка выхода из циклов, если найдено нужное время конца предшествующей обычной работы tend
//            if(tend == 0) qDebug()<<"Ошибка. Не найдено время конца предшествующей работы";
//            if(ttek <= tend) continue; // пропустить ttek, т.к. нарушается последовательность
//        }
        //НОВЕЙШИЙ ВАРИАНТ
        //ПРОВЕРКА последовательности работ. Работа не может начаться ранее, чем закончится предыдущая. 3 варианта
        //Найти tend - время, раньше которого не может начаться текущая работа job[l][m][n]
//        int tend = 0;
//        if((m==0) && (n==0)) { // Вариант 1. Если текущая работа - это начальная работа продукции
//            tend = -1000; // для этого варианта ограничение проверять не надо, т.к. нет предшествующей работы
//        }
//        else if(1==2){ // Вариант 2. Если текущая работа - это начальная работа процедуры
//            // Цикл обратного перебора всех процедур и работ l-ой продукции, начиная с работы job[l][m][n]
//            //            for(int j=job[l].size()-1; j>=0; j--){ // обратный цикл перебора процедур
//            //                for(int k=job[l][j].size()-1; k>=0; k--){ // обратный цикл перебора работ
//            //                    if((j==m) && (k==n)){
...
//            //metka22:; // метка выхода из циклов, если найдено нужное время конца предшествующей обычной работы tend
//        }
//        else { // Вариант 3. Все остальные работы (не обращаем внимание на параллельность)
//            // Цикл обратного перебора всех процедур и работ для поиска проверяемой работы job[l][m][n]
//            bool find = false; //признак того, что нужная работа пока не найдена
//            qDebug()<<"искать: l="<<l<<"m="<<m<<"n="<<n;
//            for(int j=job[l].size()-1; j>=0; j--){ // обратный цикл перебора процедур
//                for(int k=job[l][j].size()-1; k>=0; k--){ // обратный цикл перебора работ
//                    qDebug()<<"перебор всех индексов: j="<<j<<"k="<<k;
//                    if((j==m) && (k==n)){ // нашли проверяемую работу
//                        qDebug()<<"нашли нужную работу: j="<<j<<"k="<<k;
//                        find = true; // признак, нужная работа найдена
//                        continue; // пропустить саму проверяемую работу. А следующая будет предшествующая
//                    }
//                    if(find == true) { // это и есть предшествующая работа
//                        qDebug()<<"нашли предшествующую работу: j="<<j<<"k="<<k;
//                        tend = job[l][j][k].end; // конец ближайшей предшествующей любой работы
//                        qDebug()<<"tend ="<<tend;
//                        goto metka22; // прервать дальнейший поиск, т.к. tend - найдено
//                    }
//                } // конец обратного цикла перебора работ
//            } // конец обратного цикла перебора процедур
//metka22:; // метка выхода из всех циклов, если найдено tend
//        }
        //        if(!((m==0) && (n==0))){ // для начальной работы продукции нет предшествующей работы. Проверка для нее не нужна
//            int tend = 0; // искомое время предыдущей обычной работы
//            bool find = false; //признак того, что нужную работу еще не достигли припереборе работ
...
//        if(ttek <= tend) continue; // пропустить ttek, т.к. нарушается последовательность
        ...
        qDebug()<<"m="<<m<<"n="<<n<<"<<tbegin="<<tbegin<<"tend="<<tend;
...
int PlanJobEndBegin(int &l, int &m, int &n, QString &errs) // обратное планирование работы
{
    ...
    qApp->processEvents(); // показать ProgressDialog
    // Проверка. Нажата кн. Прервать? При долгом планировании его можно прервать.
    if(pprd->wasCanceled()){ // если нажата кн. Прервать
        numerror ++; // порядковый номер ошибки
        errs += QString("<br><b>%1</b>. Планирование прервано пользователем").arg(numerror);
        return 1;
    ...
    //продукцию выполнить за наименьшее время к заданному сроку.
    ...
    for(int ttek = endmax;  ; --ttek){ // бесконечный цикл перебора рабочих часов года (справа налево), начиная с максимально заданного времени конца продукции
        ...
        //+ ОФОРМИТЬ ПРОЦЕДУРОЙ ДЛЯ ПАРАЛЛЕЛЬНОЙ РАБОТЫ
        ...
        //
        //НОВЫЙ ВАРИАНТ
        //ПРОВЕРКА. Работа не может закончиться позднее, чем начнется ближайшая последующая обычная (не параллельная) работа
//        int klast = job[l][m].size()-1; // индекс k последней работы в последней процедуре
//        if(!((n==job[l][job[l].size()-1][klast]) && (m==job[l].size()-1))){ // для последней работы последней процедуры нет последующей работы. Проверка для нее не нужна
//            int tend = 0; // искомое время предыдущей обычной работы
//            bool find = false; //признак того, что нужную работу еще не достигли припереборе работ
//            // Цикл обратного перебора всех процедур и работ l-ой продукции, начиная с работы job[l][m][n]
//            for(int j=job[l].size()-1; j>=0; j--){ // обратный цикл перебора процедур
...
int Plan(TCarryTaskList &tasks, QString &errs) // основная процедура
{
    ...
    for(int i=0; i<(int)job.size(); i++){ // цикл перебора продукций
        ...
        // Планирование продукции: прямое или обратное? (в зависимости от того, заданно ли максимально разрешенное время конца продукции)
        ...
        if(job[i][0][0].begin < tbeginplan){
            numerror ++; // порядковый номер ошибки
            errs += QString("<br><b>%1</b>. Рассчитанное время начала продукции меньше текущего времени для проекта: %2").arg(numerror).arg(job[i][0][0].projectnum);
            return 1;
        }
        ...
        if(job[i][0][0].beginmin){ // если задано минимально разрешенное время начала продукции
            if(job[i][0][0].begin < job[i][0][0].beginmin){ // если рассчитанное время начала продукции меньше заданного времени начала продукции
                numerror ++; // порядковый номер ошибки
                errs += QString("<br><b>%1</b>. Рассчитанное время начала продукции меньше минимально заданного для проекта: %2").arg(numerror).arg(job[i][0][0].projectnum);
                return 1;
            }
        }
        ...
        if(job[i][0][0].endmax){ // если задано максимально разрешенное время конца продукции
            if(job[i][0][0].end > job[i][0][0].endmax){ // если рассчитанное время конца продукции больше заданного времени конца продукции
                numerror ++; // порядковый номер ошибки
                errs += QString("<br><b>%1</b>. Рассчитанное время конца продукции больше максимально заданного для проекта: %2").arg(numerror).arg(job[i][0][0].projectnum);
                return 1;
            }
        }
...
{
    numerror = 0; // обнулить порядковый номер ошибки для возвращаемого сообщения
    ...
    Progress();  // Создать ProgressDialog и внутри него ProgressBar (и кн. для прерывания планирование)

    int kodvozvrata = Plan(tasks, errs); // выполнить собственно планирование. Вернуть: 0-без ошибок, 1-есть ошибки.

    delete pprb; // удалить ProgressBar после планирования
    delete pprd; // удалить ProgressDialog после планирования

    // Проверка. Если были ошибки при планипрровании, то вернуть false. Если ошибок не было, то вернуть try
    if(kodvozvrata) return false; // вернуть false, если были ошибки
    else return true; // нет ошибок
}
