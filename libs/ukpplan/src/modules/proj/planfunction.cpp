...
// Прямое планирование (обычное), когда работы планируются слева - направо. М.б. задано минимально разрешенное время начала продукции
// Обратное планирование, когда работы планируются справа - налево. Сначала находится конец работы, а затем начало работы. Используется, когда
//задано максимальное время конца продукции
...
int TestTCarryTaskBeforePlanning(TCarryTaskList &tasks, QString &errs) // проверить объект tasks до планирования
{
    ...
    int kolplan = 0; // счетчик считанных планов
...
void FindEndBeginJob(executor_type &executor, int &tstart, int &time, int &tbegin, int &tend) // Обратное планирование. Найти конец и начало работы
{
    ...
    while(sum <= time){ // цикл, пока не найдено нужное количество рабочих часов time
        ...
        if(kol == 0) { // проверяемый час - рабочий? (т.к. он не найден среди списка нерабочих часов)
            ...
            //if(sum == 1)    tbegin = ttek; // запомнить первый найденный рабочий час, как tbegin
            if(sum == 1)    tend = ttek; // запомнить первый найденный рабочий час, как tbegin

            //if(sum == time) tend   = ttek; // запомнить последний найденный рабочий час, как tend
            if(sum == time) tbegin   = ttek; // запомнить последний найденный рабочий час, как tend
        }
        //++ttek; // переход к следующему часу для его проверки. Двигаемся слева направо
...
void FillVectorJob(TCarryTaskList &tasks) // заполнить данные о работах
{
    ...
    foreach (TCarryTask *curTask, tasks){ // цикл перебора проектов
        ...
        //ПРОБА. В цикле перебрать планы
        //qDebug()<<"-------------------";
        //QList<QObject*> list = curTask->children();
        //while(!list.isEmpty()){
        //    qDebug()<<"list.takeFirst()->objectName()"<<list.takeFirst()->objectName();
        //}
        //
...
int PlanProductEndMax(int &i, QString &errs){ // СТАРЫЙ МЕДЛЕННЫЙ ВАРИАНТ спланировать продукцию с заданным максимально возможным временем конца
    ...
    //На будущее. Если время выполнения этой процедуры станет большим (из за множества итераций), то надо будет написать новые алгоритмы
    //по аналогии с "прямым"  пересчетом и время выполнения станет минимальным, т.к. будет всего одна итерация.
    //Здесь - главный резерв программы для повышения быстродействия!
    ...
    if(PlanProductBeginEnd(i, errs) != 0) return 1; // спланировать продукцию для нахождения конца продукции
...
int Plan(TCarryTaskList &tasks, QString &errs) // основная процедура
{
    ...
    for(int i=0; i<(int)job.size(); i++){ // цикл перебора продукций
        ...
        if(job[i][0][0].endmax){ // Да, есть максимально разрешенное время конца продукции
            //if(PlanProductEndMax(i, errs)) return 1;   // прямое планирование. Использование итераций. Долго решается. Старый вариант
