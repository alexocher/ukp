...
// Максимально разрешенное время конца продукции - время, до которого продукции должна быть выполнена. Если это требование невыполнимо -
//планирование отменяется.
...
// Сущность основного алгоритма планирования заключается в последовательном планировании продукции, начиная с продукции с наивысшим приоритетом.
//Недостаток алгоритма - точное решение не находится, только приближенное. Достоинство - быстрота (нет полного перебора вариантов).
// Принцип "последовательности" выполнения работ означает, что работа не может начаться раньше, чем закончится предыдущая (в рамках одной продукции).
// Принцип "дублирования" работ означент, что один исполнитель не может одновременно выполнять более одной работы.
// Приоритет продукции (по-умолчанию = 0) позволяет выстроить последовательность планирования продукции. Чем выше приоритет, тем быстрее будет
...
// Приоритет работы соответствует приоритету проекта, которому принадлежит работа
// Защищенная работа (процедура, продукция) не может быть изменена в процессе планирования. Использование защищенных работ требует навыка и
...
namespace {
    ...
    int step;                        // счетчик спланированных работ для ProgressBar
    int kolproc;                     // общее количество процедур в векторе job для ProgressBar
...
void Progress() // настройка ProgressBar
{
    ...
    //ProgressBar - показывает ход обработки процедур, а не работ, т.к. количество обрабатываемых процедур известно, а работ нет.
    //Количество обрабатываемых работ зависит от количества возможных исполнителей работ.
    // Только после считывания данных из tasks становится известно общее количество процедур kolproc, которое используется в ProgressBar
    ...
    pprb->setTextVisible(true);            // показывать цифры процентов выполнения ProgressBar
    pprb->setMaximum(kolproc);             // установить максимум ProgressBar (по количеству процедур в векторе job)
    pprb->setMinimum(0);                   // установить минимум ProgressBar
    pprb->setValue(0);                     // установить начальное значение ProgressBar
    pprd->setBar(pprb);                    // установить ProgressBar внутрь ProgressDialog
    pprd->setMinimumDuration(1);           // не показывать окно ProgressDialog, если программа выполняется менее 1 милисекунд
...
int TestTCarryTaskBeforePlanning(TCarryTaskList &tasks, QString &errs) // проверить объект tasks до планирования
{
    ...
    foreach (TCarryTask *curTask,tasks) // цикл перебора проектов
    {
        ...
        //+
        //QString name  = curTask->name();
        //qDebug()<<"curTask->name()="<<name.toStdString().c_str();
        //qDebug().noquote()<<"curTask->name()="<<curTask->name();
        //-
...
int TestData(QString &errs) // тестировать job
{
...
//    }
    ...
    if(job.size() == 0) {
        if(flprint>=1) qDebug("Ошибка. Вектор job (сведения о работах) пустой");
        errs += QString("<br> Не найдена работа");
    }
...
int TestPlan(QString &errs) // тестировать сформированный план
{
    // Тестирование сформированного плана на ошибки. Тестирование вектора job после планирования

    //Проверка выполнения принципа последовательности выполнения работ одной продукции.
    ...
    for(int i=0; i<(int)job.size(); ++i){ // цикл перебора продукций
        for(int j=0; j<(int)job[i].size(); ++j){ // цикл перебора процедур
            for(int k=0; k<(int)job[i][j].size(); ++k) { // цикл перебора работ
                ...
                if((j==0) && (k==0)){ // это начальная работа продукции
                ...
                if(job[i][j][k].begin < endold){
                    //QMessageBox::information(0,"Отладка1", "Нарушение последовательности работ в плане");
                    numerror ++; // порядковый номер ошибки
...
void SortingVectorJob() // сортировка продукций в job
{
            ...
            foreach (TCarryProcedure *proc, plan->procedures()){ // цикл перебора процедур
                ...
                foreach (TCarryWork *work, proc->works()){ // цикл перебора работ
                    ...
                    if(jobtemp.protection){ // защищенная работа. В ней ничего менять нельзя
                        ...
                        //+ 06.06.2017
                        // Проверка. Если в защищенной работе есть исполнитель, но список возможных исполнителей - пуст, то
                ...
                ++kolproc; // счетчик общего числа процедур, записанных в вектор job (для установки ProgressDar)
            } // конец цикла перебора процедур
        ...
        for(int j=0; j<(int)job[i].size(); j++){ // цикл перебора процедур
            for(int k=0; k<(int)job[i][j].size(); k++){ // цикл перебора работ
                ...
                qDebug()<<"i:"                <<i
                        ...
                        <<"people.code:"      <<b; // это список кодов возможных исполнителей
            } // конец цикла перебора работ
        } // конец цикла перебора процедур
...
int PlanJobBeginEnd(int &l, int &m, int &n, QString &errs) // прямое планирование работ
{
    ...
    for(int ttek = tbeginplan;  ; ++ttek){ // бесконечный цикл перебора рабочих часов года, начиная с текущего рабочего часа
        ...
        //СТАРЫЙ ВАРИАНТ, когда не было параллельных процедур и работ. Быстрый!
        ...
        if(m!=0 && n==0){  // это начальная работа не начальной процедуры. Надо найти конец последней работы предыдущей процедуры
...
//        }
//        //НОВЕЙШИЙ ВАРИАНТ (быстрый, нет лишнего перебора циклов)
//        // Найти tend - конец предшествующей работы. Нельзя просто взять индекс "n-1". Надо учесть переходы из одной в др. процедуру
//        if((m==0) && (n==0)) { // Вариант 1. Если текущая работа - это начальная работа продукции
//        } // ничего не проверять, т.к. для этой работы нет предшествующей
//        else {                 // Вариант 2. Все остальные работы
//            // Цикл обратного перебора всех процедур и работ для поиска проверяемой работы job[l][m][n]
//            int step = 0;    // счетчик шагов
//            for(int j=m; j>=0; j--){ // обратный цикл перебора процедур
//                for(int k=n; k>=0; k--){ // обратный цикл перебора работ
//                    if(step == 0){ // это текущая работа job[l][m][n]
//                        step = step + 1; // счетчик шагов
//                        continue;
//                    }
//                    else{ // это искомая предшествующая работа
//                        tend = job[l][j][k].end; // конец ближайшей предшествующей работы
//                        goto metka22; // прервать дальнейший поиск, т.к. tend - найдено
//                    }
//                } // конец обратного цикла перебора работ
//            } // конец обратного цикла перебора процедур
...
int PlanProductBeginEnd(int &i, QString &errs){ // прямое планирование продукции
    ...
    for(int j=0; j<(int)job[i].size(); ++j){ // цикл перебора процедур
        ...
        pprb->setValue(++step); // установить "ползунок", где step - счетчик обработанных процедур
        qApp->processEvents();  // показать ProgressDialog и ProgressBar
...
int Plan(TCarryTaskList &tasks, QString &errs) // основная процедура
{
    ...
    Progress();                                             // cоздать ProgressDialog и внутри него ProgressBar (и кн. для прерывания планирование)
    ...
    // Планирование продукций. М.б. прямое или обратное
    for(int i=0; i<(int)job.size(); i++){ // цикл перебора продукций

        // Проверка. Какое планирование продукции: прямое или обратное? (в зависимости от того, заданно ли максимально разрешенное время конца продукции)
        ...
        if(job[i][0][0].begin < tbeginplan){
            numerror ++; // порядковый номер ошибки
            errs += QString("<br><br><b>%1</b>. Рассчитанное время начала продукции меньше текущего времени для проекта: %2. %3").
...
bool TModulePlans::createCarryPlans(TCarryTaskList &tasks, QString &errs)
{
    ...
    step = 0;     // счетчик процедур для работы ProgressBar
    kolproc = 0;  // общее количество процедур в векторе job для работы ProgressBar
