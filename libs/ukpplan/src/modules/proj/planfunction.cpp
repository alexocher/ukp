...
//
// "Обычная" работа (ОР) и "параллельная" работа (ПР). ПР - это работа, которая позволяет выполнять последующие работы (одну или несколько, ПР или ОР)
//параллельно текущей ПР. В этом случае не будет выполняться жесткий принцип последовательности работ одной продукции. Унивесальное правило - любая
//работа(ПР или ОР) может начинаться только после окончания ближайшей предыдущей ОР. Для каждой ПР будет искусственно создаваться новая
//процедура, содержащая только одну эту ПР. Других работ в этой новой процедуре - не будет. Это позволит использовать одни и те же алгоритмы для
//обработки и ПР и ОР. Несколько ПР могут выполняться параллельно. Для того, что бы в процедуре, содержащей ПР, осуществилась действительно
//паралллельность работ необходимо не менее 2-ух возможных исполнителей процедуры. Если возможный исполнитель будет один, то никакой параллельности
//не будет, т.к. один исполнитель не может одновременно выполнять несколько работ. При этом, ошибки не будет, просто работы будут выполняться
//последовательно.
//
...
namespace {
    ...
    int   flprint         = 0;       // флаг вывода отладочной информации в консоль: 0-ничего; 1-ошибки,план; 2-ошибки,план,job; 3-ошибки,план,job,tasks, paraljob
    ...
    struct job_type { // структура работы
        ...
        //int                    proceduretype; // тип процедуры. 0 - по-умолчанию; 1 - тип 1; 2 - тип 2; 3-тип 3. 1 - удаляются все работы до параллельной.
                                              //2 - удаляются все работы кроме параллекльной; 3 - удаляются все работы после параллельной
                                              //??? ИПОЛЬЗУЕТСЯ ЛИ ???
        ...
        //bool                   paral;         // параллельность работы (true - параллельная, false - обычная)
        int                   paral;         // параллельность работы (0-обычная; 1-параллельная необработанная; 2-параллельная обработанная)
    };
    ...
    for(int i=0; i<(int)job.size(); ++i){ // цикл перебора продукций
        for(int j=0; j<(int)job[i].size(); ++j){ // цикл перебора процедур
            for(int k=0; k<(int)job[i][j].size(); ++k) { // цикл перебора работ
                ...
                if(job[i][j][k].paral >0) continue; // для параллельной работы не проверять последовательность (т.к. может нарушаться)
...
void ParalJob() // обработка параллельных работ
{
        ...
        for(int i=0; i<=1; i++){ // цикл перебора продукций (двух планов): План выпуска продукции (i=0) и План ОРД (i=1)
            ...
            foreach (TCarryProcedure *proc, plan->procedures()){ // цикл перебора процедур
                ...
                foreach (TCarryWork *work, proc->works()){ // цикл перебора работ
                    ...
                    //jobtemp.proceduretype = 0;                   // тип процедуры (м.б. 0, 1, 2, 3)
                    ...
                    //+
                    //jobtemp.paral        = work->???        // параллельность работы
                    //if(work->planPeriod() == 29) jobtemp.paral= true; // ??? УДАЛИТЬ. Только для отладки
                    //else jobtemp.paral = false;                       // ??? УДАЛИТЬ. Только для отладки
                    if(work->planPeriod() == 29) jobtemp.paral= 1; // ??? УДАЛИТЬ. Только для отладки
                    else jobtemp.paral = 0;                       // ??? УДАЛИТЬ. Только для отладки
                    //-
        ...
        for(int j=0; j<(int)job[i].size(); j++){ // цикл перебора процедур
            for(int k=0; k<(int)job[i][j].size(); k++){ // цикл перебора работ
                ...
                qDebug()<<"i:"                <<i
                        ...
                        //<<"proc type:"        <<job[i][j][k].proceduretype
...
int PlanJobBeginEnd(int &l, int &m, int &n, QString &errs) // прямое планирование работы
{
    ...
    for(int ttek = tbeginplan;  ; ++ttek){ // бесконечный цикл перебора рабочих часов года, начиная с текущего рабочего часа
        ...
        //СТАРЫЙ ВАРИАНТ, когда не было параллельных работ
...
int Plan(TCarryTaskList &tasks, QString &errs) // основная процедура
{
    ...
    if(flprint>=2) PrintJob();                              // для отладки вывести в консоль данные вектора job в табличной форме до начала планирования
                                                            //до обработки параллельности

    if(flprint>=3) PrintParalJob();                         // для отладки вывести в консоль работы job до обработки параллельности
    ParalJob();                                             // обработать параллельные работы
    if(flprint>=3) PrintParalJob();                         // для отладки вывести в консоль работы job после обработки параллельности

    if(flprint>=2) PrintJob();                              // для отладки вывести в консоль данные вектора job в табличной форме до начала планирования
                                                            //после обработки параллельности
