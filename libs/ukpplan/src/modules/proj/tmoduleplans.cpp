...
void TModulePlans::fillTasks(QList<int> tskids)
{
    ...
    {
        int curYear(0);
        QString syear(PROJ->findParam("-year"));
        if (syear!="NOTFOUND") curYear = syear.toInt();
        if (!curYear) curYear = QDate::currentDate().year();
        try
        {
            ...
            if (yearPlan != 0)
            {
                ...
                for(EM_BasePlanItem::iterator projIter = root->begin(); projIter != root->end(); ++projIter)
                {
                    ...
                    if (EM_ProjectPlanItem *curProject = dynamic_cast<EM_ProjectPlanItem*>(*projIter)) // проект
                    {
                        ...
                        newCarryTask->setSSort(newCarryTask->scrName());
                    ...
                    qStableSort(fPlCarryTasks.begin(), fPlCarryTasks.end(), obj::FSortMinToMaxStr<TCarryTask*>());
                }
                delete yearPlan;
            }
        }
